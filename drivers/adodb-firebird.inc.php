...
class ADODB_firebird extends ADOConnection {
	...
	{  
		...
		if (function_exists('fbird_timefmt')) {
			...
			if ($this->dialect == 1) fbird_timefmt($this->fbird_datefmt,fbird_TIMESTAMP );
			else fbird_timefmt($this->fbird_timestampfmt,fbird_TIMESTAMP );
	...
	{ 
		...
		if ($docommit && $ret === true) fbird_commit($this->_connectionID);
	 ...
	 function _close()
	 {	   
		if (!$this->autoCommit) @fbird_rollback($this->_connectionID);
