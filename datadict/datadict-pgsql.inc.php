...
class ADODB2_pgsql extends ADODB_DataDict {
	 ...
	 * Adding a new Column 
	 *
	 * reimplementation of the default function as pgsql does NOT allow to set the default in the same statement
	 *
	 ...
	 *
	 * pgsql can't do that on it's own, you need to supply the complete defintion of the new table,
	 * to allow, recreating the table and copying the content over to the new table
	 * @param string $tabname table-name
	 * @param string $flds column-name and type for the changed column
	 * @param string $tableflds complete defintion of the new table, eg. for pgsql, default ''
	...
	{
		if (!$tableflds) {
			if ($this->debug) ADOConnection::outp("AlterColumnSQL needs a complete table-definiton for pgsqlQL");
			return array();
		}
	 ...
	 * Drop one column
	 *
	 * pgsql < 7.3 can't do that on it's own, you need to supply the complete defintion of the new table,
	 * to allow, recreating the table and copying the content over to the new table
	 * @param string $tabname table-name
	 * @param string $flds column-name and type for the changed column
	 * @param string $tableflds complete defintion of the new table, eg. for pgsql, default ''
	...
	function DropColumnSQL($tabname, $flds, $tableflds='',$tableoptions='')
	{
		...
		if (!$has_drop_column && !$tableflds) {
			if ($this->debug) ADOConnection::outp("DropColumnSQL needs complete table-definiton for pgsqlQL < 7.3");
		return array();
	}
		if ($has_drop_column) {
	 ...
	 * @param string $tableflds complete defintion of the new table, eg. for pgsql
	...
	// this is still necessary if pgsql < 7.3 or the SERIAL was created on an earlier version!!!
